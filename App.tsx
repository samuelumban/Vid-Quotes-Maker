...
const App: React.FC = () => {
  ...
  }, [isPlaying, isRecording, design.audioStart, design.audioEnd]);
  ...
  // --- Logic for AI Generation (Promisified) ---
  
  const generateAiBackground = async (prompt: string): Promise<void> => {
     if (!prompt) return;
     
     const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
     // Explicitly request an image to ensure the model behaves correctly
     ...
     if (base64Image) {
         return new Promise((resolve, reject) => {
             const img = new Image();
             img.onload = () => {
                 updateDesign({ bgImage: img, bgType: 'image' }, false); // Don't commit AI changes to undo history during batch to save memory
                 resolve();
             };
             img.onerror = reject;
         ...
         throw new Error(`No image generated. Model response: "${textContent || 'No text output'}"`);
     }
  };

  const handleAiGenerate = async () => {
      if (!aiPrompt.trim()) {
          alert("Please enter a description for the background in the AI Gen tab first.");
          setActiveTab('ai');
          return;
      }
  ...
  const drawWeather = (ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    ctx.save();
    ctx.globalCompositeOperation = design.weatherType === 'confetti' ? 'source-over' : 'screen';
    ...
    particlesRef.current.forEach(p => {
        ...
        if (design.weatherType === 'snow') {
            ...
            ctx.beginPath();
            ctx.arc(p.x, p.y, p.size * design.weatherScale, 0, Math.PI * 2);
            ctx.fill();
             ...
             ctx.beginPath();
             ctx.rect(0, 0, 1 * Math.max(0.5, design.weatherScale * 0.5), p.size * design.weatherScale * 5);
             ctx.fill();
             ctx.restore();

        } else if (design.weatherType === 'confetti') {
             p.x += Math.sin(p.wobble) * 2 * design.weatherWobble;
             p.wobble += 0.1;
             ...
             const size = p.size * design.weatherScale;
    ...
    <div className="flex flex-col h-screen w-full bg-gray-950 text-white font-sans overflow-hidden">
      ...
      <div className="h-16 shrink-0 bg-gray-850/90 backdrop-blur border-b border-gray-750 flex items-center justify-between px-4 lg:px-8 z-20 shadow-xl relative">
          ...
          <div className="flex items-center gap-6">
            ...
            {/* Quick Actions (Replaces Batch Control) */}
            <div className="ml-4 flex items-center gap-2">
                <button 
                    onClick={loadNextQuote} 
                    disabled={quotes.length === 0 || isRecording}
                    className={`
                        ...
                        <div className="flex flex-wrap gap-3">
                            {design.colors.map((color, index) => (
                                <div key={index} className="relative group">
                                    ...
                                    {design.colors.length > 2 && <button onClick={() => removeColor(index)} disabled={isRecording} className="absolute -top-1 -right-1 bg-gray-900 text-gray-400 hover:text-red-400 rounded-full p-0.5 opacity-0 group-hover:opacity-100 border border-gray-700"><X size={10} /></button>}
                                </div>
                            ))}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-[1fr_2fr] gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="bg-gray-800/50 rounded-lg p-3 border border-gray-800 flex flex-col gap-2 h-full max-h-[200px] overflow-y-auto">
                        <div className="flex justify-between items-center mb-2 px-1">
                            ...
                            <button onClick={addTextLayer} disabled={isRecording} className="p-1 hover:bg-gray-700 rounded text-blue-400"><Plus size={14} /></button>
                        </div>
                        {design.textLayers.map((layer, i) => (
                            ...
                            >
                                ...
                                <button onClick={(e) => { e.stopPropagation(); removeTextLayer(layer.id); }} disabled={isRecording} className="text-gray-500 hover:text-red-400 p-1"><Trash2 size={12} /></button>
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <select value={activeLayer.fontFamily} onChange={(e) => { updateActiveLayer({ fontFamily: e.target.value }); handleCommit(); }} disabled={isRecording} className="w-full bg-gray-800 text-xs border border-gray-700 rounded px-2 py-2 text-gray-300">
                                        ...
                                        <label className={`flex items-center justify-center p-2 bg-gray-700 hover:bg-gray-600 rounded cursor-pointer border border-gray-600 ${(isRecording) ? 'opacity-50 pointer-events-none' : ''}`} title="Upload Font (TTF, OTF, WOFF)">
                                            <Upload size={14} className="text-gray-300" />
                                            <input type="file" accept=".ttf,.otf,.woff" onChange={handleFontUpload} className="hidden" disabled={isRecording} />
                                        </label>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="flex bg-gray-800 rounded border border-gray-700 p-1">
                                        {(['left', 'center', 'right'] as const).map(align => (
                                            <button key={align} onClick={() => { updateActiveLayer({ textAlign: align }); handleCommit(); }} disabled={isRecording} className={`flex-1 py-1 rounded text-xs capitalize ${activeLayer.textAlign === align ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-gray-300'}`}>
                                                {align}
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="range" min="0" max="1" step="0.01" value={activeLayer.x} onChange={(e) => updateActiveLayer({ x: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                </div>
                                <div>
                                    <label className="block text-xs text-gray-400 font-bold uppercase mb-2">Position Y</label>
                                    <input type="range" min="0" max="1" step="0.01" value={activeLayer.y} onChange={(e) => updateActiveLayer({ y: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4">
                                <div className="flex-1">
                                    ...
                                    <input type="range" min="20" max="400" value={activeLayer.fontSize} onChange={(e) => updateActiveLayer({ fontSize: parseInt(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                </div>
                                <div className="flex items-center gap-3">
                                    <div>
                                        <label className="block text-xs text-gray-400 font-bold uppercase mb-2">Color</label>
                                        <input type="color" value={activeLayer.textColor} onChange={(e) => updateActiveLayer({ textColor: e.target.value })} onBlur={handleCommit} disabled={isRecording} className="w-8 h-8 rounded cursor-pointer bg-transparent border-2 border-gray-700" />
                                    </div>
                                     <div>
                                        <label className="block text-xs text-gray-400 font-bold uppercase mb-2">Opacity</label>
                                        <input type="range" min="0" max="1" step="0.1" value={activeLayer.opacity} onChange={(e) => updateActiveLayer({ opacity: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-20 h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="flex flex-col items-center justify-center bg-gray-800/30 border-2 border-dashed border-gray-700 rounded-lg p-8 relative hover:border-blue-500 transition-colors">
                        {design.logo ? (
                            <div className="relative w-full h-full flex items-center justify-center">
                                ...
                                <button onClick={removeLogo} disabled={isRecording} className="absolute top-0 right-0 bg-red-500/20 text-red-400 p-1.5 rounded-full hover:bg-red-500 hover:text-white transition-colors">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        ) : (
                        ...
                        <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'logo')} className="absolute inset-0 opacity-0 cursor-pointer" disabled={isRecording} />
                    </div>
                    ...
                    <div className={`space-y-6 ${!design.logo ? 'opacity-50 pointer-events-none' : ''}`}>
                         <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs text-gray-400 font-bold uppercase mb-2">Position X</label>
                                    <input type="range" min="0" max="1" step="0.01" value={design.logo?.x || 0.5} onChange={(e) => updateLogo({ x: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                </div>
                                <div>
                                    <label className="block text-xs text-gray-400 font-bold uppercase mb-2">Position Y</label>
                                    <input type="range" min="0" max="1" step="0.01" value={design.logo?.y || 0.5} onChange={(e) => updateLogo({ y: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                </div>
                         </div>
                         <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs text-gray-400 font-bold uppercase mb-2">Size</label>
                                    <input type="range" min="0.05" max="1" step="0.01" value={design.logo?.size || 0.2} onChange={(e) => updateLogo({ size: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                </div>
                                <div>
                                    <label className="block text-xs text-gray-400 font-bold uppercase mb-2">Opacity</label>
                                    <input type="range" min="0" max="1" step="0.05" value={design.logo?.opacity || 1} onChange={(e) => updateLogo({ opacity: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg accent-blue-500" />
                                </div>
                         </div>
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                     ...
                                     <input type="range" min="10" max="100" value={design.weatherDensity} onChange={(e) => updateDesign({ weatherDensity: parseInt(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                                </div>
                                <div className="space-y-2">
                                     ...
                                     <input type="range" min="0.2" max="3.0" step="0.1" value={design.weatherScale} onChange={(e) => updateDesign({ weatherScale: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                                </div>
                                <div className="space-y-2">
                                    <div className="flex justify-between">
                                    ...
                                    <input type="range" min="0.1" max="5.0" step="0.1" value={design.weatherSpeed} onChange={(e) => updateDesign({ weatherSpeed: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                     ...
                                     <input type="range" min="-45" max="45" value={design.weatherAngle} onChange={(e) => updateDesign({ weatherAngle: parseInt(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                ...
                                <div className="space-y-2">
                                     ...
                                     <input type="range" min="0" max="5.0" step="0.1" value={design.weatherWobble} onChange={(e) => updateDesign({ weatherWobble: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                </div>
                                <div className="space-y-2">
                                    <div className="flex justify-between">
                                    ...
                                    <input type="range" min="0" max="1.5" step="0.1" value={design.weatherOpacity} onChange={(e) => updateDesign({ weatherOpacity: parseFloat(e.target.value) })} onMouseUp={handleCommit} disabled={isRecording} className="w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                </div>
                            </div>
                        </div>
                    )}
            ...
            {activeTab === 'ai' && (
                <div className="max-w-3xl mx-auto grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="space-y-4">
                        ...
                        <button 
                            onClick={handleAiGenerate}
                            disabled={isGenerating || !aiPrompt || isRecording}
                            className={`w-full py-3 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all ${isGenerating || !aiPrompt || isRecording ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-400 hover:to-purple-500 text-white shadow-lg'}`}
                        >
